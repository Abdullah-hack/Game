            ...
            for (int i = 0; i < collidables.Count; i++)
            {
                for (int j = i + 1; j < collidables.Count; j++)
                {
                    if (collidables[i].Bounds.IntersectsWith(collidables[j].Bounds))
                    {
                        ...
                        // Ignore bullet vs bullet
                        //if (a is Bullet && b is Bullet)
                        //    continue;


                        // Ignore bullet vs owner
                        //if (a is Bullet bulletA && bulletA.Owner == b)
                        //    continue;

                        //if (b is Bullet bulletB && bulletB.Owner == a)
                        //    continue;

                        //if (a.Bounds.IntersectsWith(b.Bounds))
                        //{
                        //    // Bullet involved → logic only
                        //    if (a is Bullet || b is Bullet)
                        //    {
                        //        a.OnCollision(b);
                        //        b.OnCollision(a);
                        ...
                        //}
                            ...
                            if (a is Bullet bulletA && b is Bullet bulletB)
                            {
                                if (bulletA.Owner != bulletB.Owner)
                                {
                                    continue;
                                }
                            }
                        ...
                        // 🔥 Ignore bullet vs bullet completely
                        if (a is Bullet && b is Bullet)
                            continue;

                        // 🔥 Ignore bullet vs owner
                        if (a is Bullet ba && ba.Owner == b)
                            continue;

                        if (b is Bullet bb && bb.Owner == a)
                            continue;
                        ...
                        // 🔥 Bullet involved → NO physics resolution
                        if (a is Bullet || b is Bullet)
                        {
                            a.OnCollision(b);
                            b.OnCollision(a);
